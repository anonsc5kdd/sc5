...
def evaluate(
    ...
    plot_batch_col: Union[str, None] = "batch_indices",
    color_by: Iterable[str] = None,
    cmap: Union[dict, None] = None,
    ...
    if cell_type_col and not pd.api.types.is_categorical_dtype(
        ...
        adata.obs[cell_type_col] = (
            adata.obs[cell_type_col].astype(str).astype("category")
        )
    if plot_batch_col and not pd.api.types.is_categorical_dtype(adata.obs[plot_batch_col]):
        # _logger.warning("scETM.evaluate assumes discrete batches. Converting plot_batch_col to categorical.")
        adata.obs[plot_batch_col] = adata.obs[plot_batch_col].astype(str).astype("category")
    ...
    if cell_type_col in adata.obs and len(resolutions) > 0:
        cluster_key, best_ari, best_nmi = clustering(
            ...
            batch_col=plot_batch_col,
            clustering_method=clustering_method,
        )
        if 'baseline' in embedding_key:
            return {
                f'{plot_fname}ari':best_ari,
                f'{plot_fname}nmi':best_nmi,
                f'{plot_fname}resolution':cluster_key,
            }
    ...
    print('plotting umap')
    need_batch = plot_batch_col and adata.obs[plot_batch_col].nunique() > 1
    # plot UMAP embeddings
    if return_fig or plot_dir is not None:
        if color_by is None:
            color_by = [plot_batch_col, cell_type_col] if need_batch else [cell_type_col]
        ...
        fig = draw_embeddings(
            ...
            spread=spread,
            ckpt_dir=plot_dir,
            fname=f"{plot_fname+str(n_epoch)}.{plot_ftype}",
            title=title,
...
def _entropy_batch_mixing_for_one_pool(
    batches, knn_indices, nsample, n_samples_per_pool
):
    indices = np.random.choice(np.arange(nsample), size=n_samples_per_pool)
    return np.mean(
        [_entropy(batches[knn_indices[indices[i]]]) for i in range(n_samples_per_pool)]
    )
...
def draw_embeddings(
    ...
    named_colors = list(plt.cm.colors.cnames.keys())

    try:
        fig = sc.pl.umap(adata, wspace=0.5, title=title,  color='cell_type', palette=cmap, show=False, return_fig=True, **umap_kwargs)
    except:
        import ipdb ; ipdb.set_trace()

    print("saving to", ckpt_dir, fname)
    if ckpt_dir is not None:
        assert os.path.exists(ckpt_dir), f"ckpt_dir {ckpt_dir} does not exist."
        fig.savefig(os.path.join(ckpt_dir, fname), dpi=dpi, bbox_inches="tight")
    if return_fig:
        return fig
    fig.clf()
    plt.close(fig)
